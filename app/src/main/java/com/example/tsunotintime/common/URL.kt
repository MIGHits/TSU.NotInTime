package com.example.tsunotintime.common

object URL {
    const val BASE_URL = "http://88.204.10.182:5693/api/"
    const val IMAGE_URL = "http://88.204.10.182:5693/"
    const val GET_CONCRETE_REQUEST = "request/{id}"
    const val REQUEST = "request"
    const val GET_TARGET_USER_REQUESTS = "request/user/{userId}"
    const val REASON = "https:/88.204.10.182:5693/"
    const val LOGIN_URL = "user/login"
    const val REGISTER_URL = "user/register"
    const val REFRESH_URL = "user/refresh-token"
    const val LOGOUT_URL = "user/logout"
    const val PROFILE_URL = "user/profile"

    //const val IMAGE_URL =  "http://88.204.10.182:5693/"
    //"http:/10.0.2.2:5693/api/"
}
